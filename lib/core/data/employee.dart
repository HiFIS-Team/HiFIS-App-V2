...
enum Role {
  ...
  /// **현장 근무를 안 하고 운영만 보는 사람인가** — MASTER · ADMIN
  ///
  /// 이 둘은 출퇴근을 안 찍고 매출·수업 실적도 없어서, **본인 것을 보여주는
  /// 자리가 늘 비어 있다.** 그래서 여러 화면이 그 자리를 관리용으로 바꿔 끼운다.
  ///
  /// | 화면 | 빈 자리 | 대신 놓는 것 |
  /// |---|---|---|
  /// | 홈 | 오늘 근무 카드 | 결재 대기 · 오늘 출근 |
  /// | 근태 | 달 요약 · 쓴 월차 | 전 직원 현황 · 월차 결재 |
  /// | 급여 | 급여 신청서 작성 | 급여 결재함 |
  /// | 랭킹 | 내 순위 | 추월 기록 |
  /// | 기여도 | 내가 받은 내역 | 내가 준 내역 |
  ///
  /// **MANAGER 는 여기 안 든다** — 점장도 현장 근무를 해서 본인 자리가 채워진다.
  ///
  /// [strong] 과 헷갈리지 않는다. 저쪽은 *무엇을 할 수 있나*(MANAGER 포함),
  /// 이쪽은 *본인 기록이 있나*(MANAGER 제외)다. 값이 다르고 뜻도 다르다.
  ///
  /// 예전에는 이 판정이 여덟 화면에 `master || admin` 으로 각자 적혀 있었고
  /// 이름이 `_isBoss` · `_isRankBoss` · `_isPayBoss` · `_givenOnly` ·
  /// `_canSeeOthers` 로 갈렸다. 규칙이 바뀔 때 한 곳만 빠뜨려도 그 화면만
  /// 조용히 틀리는 자리라 여기로 모았다 (2026-08-28).
  bool get boss => this == Role.master || this == Role.admin;
