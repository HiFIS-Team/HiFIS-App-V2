...
/// 근무일에 한 번 더 서고, **그 날까지도 안 하면** 확정되어 점수가 깎인다.
/// 사유서를 내서 승인받으면 되돌아온다.
///
/// **얼마를 깎는지는 그날 빠뜨린 항목 수로 정해진다** — 하루치 기본 -20 에
/// 항목이 하나 늘 때마다 -10 이 더 붙는다 (서버 `task_miss_points`).
/// 2026-09-16 에 누적(-10/-20/-30)으로 갈았다가 이틀 만에 걷어냈다
/// (2026-09-18 대표 결정) — 축이 둘이면 본인이 점수를 미리 셀 수 없다.
class MyTaskMiss {
  MyTaskMiss({
    required this.id,
    required this.employeeId,
